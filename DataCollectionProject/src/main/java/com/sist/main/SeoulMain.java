...
public class SeoulMain {
			...
			for(int i=1;i<=35;i++)
			{
			   ...
			   for(int j=0;j<poster.size();j++)
			   {
				   try{
					   ...
					   Elements address=doc2.select("div.detail-map-infor dl dt");
					   String add="";
					   for(int m=0;m<address.size();m++)
					   {
						   String addr=address.get(m).text();
						   if(addr.equals("주소"))
						   {
							   System.out.println("addr="+addr);
							   Element a=doc2.select("div.detail-map-infor dl dt+dd").get(m);
	...
	public void seoulShop()
	{
		...
		try
		{
			// https://korean.visitseoul.net/shopping?curPage=28
			int k=1;
			for(int i=1;i<=28;i++)
			{
